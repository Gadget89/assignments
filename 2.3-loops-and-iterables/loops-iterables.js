...
function squareDance(numbersArray){
  ...
  for(var i = 0; i < numbersArray.length; i++){
    //Hold a reverence to current number
...
//I don't quite understand what the number in the second array is in each line below. I know we reversed engineered this to to make the function to work. I think it represents the plae holder in the array input to be determined by the function.
...
function properSentences(paragraph){
  ...
  for (var i=0; i < paragraphArray.length; i++){
    ...
    var sentenceWordArray = currentSentence.split("");
    var capFirstLetter = sentenceWordArray[0].toUpperCase();
    sentenceWordArray.splice(0, 1);
    sentenceWordArray.unshift(capFirstLetter);
    var currentWordCap = sentenceWordArray.join("");
    //The code until this poin has changed the first letter of each sentence to upper case but the first two sentences are missing the periods at the end.
    //if (i === paragraphArray.length - 1){
    //  sentenceCap += currentWordCap;
    //}else{
    //  sentenceCap += currentWordCap + " ";
    //}
    console.log(currentWordCap);
  }
  //return sentenceCap;
}
